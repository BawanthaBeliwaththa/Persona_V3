let activeProfileData = null;
            ...
            async function searchAndExtract() {
                ...
                if (!firstName && !lastName) { showSearchError('Enter username, name, or contact info'); return; }
            ...
            async function clientUploadFile() {
                ...
                const btn = document.getElementById('btnClientUpload');
                btn.disabled = true;
                ...
                try {
                    ...
                    btn.disabled = false;
                    btn.innerHTML = '<i class="fas fa-upload"></i> Upload & Queue';
                }
            }
            ...
            function handleRefLookupSuccess(data) {
                ...
                // Store profiles on a module-level variable so download fns can access them
            ...
            function renderProfile(profile) {
                ...
                <div class="profile-title-details">
                    ...
                    ${profile.connections ? `<p style="color:var(--text-muted); font-size:0.85rem; margin-top:4px;"><i class="fas fa-users" style="color:var(--secondary-accent);"></i> ${escapeHtml(profile.connections)}</p>` : ''}
                    <a href="${escapeHtml(profile.profile_url)}" target="_blank" style="color:var(--secondary-accent); font-size:0.85rem;"><i class="fab fa-linkedin"></i> View on LinkedIn</a>
                </div>
            `;
            ...
            async function exportData(format, isBulk = false) {
                const data = isBulk ? { profiles: activeBulkData } : { profiles: [activeProfileData] };
                ...
                try {
                    ...
                    if (res.ok) {
                        ...
                        const a = document.createElement('a');
                        a.href = url;
                        a.download = `linkedin_export_${isBulk ? 'bulk' : 'profile'}.${format}`;
                        a.click();
                        URL.revokeObjectURL(url);
                    } else {
            ...
            async function exportPDF() {
                if (!activeProfileData) { showSearchError('No profile loaded to export.'); return; }
                try {
                    const res = await fetch('/api/export-profile-pdf', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ profile: activeProfileData })
                    });
                    if (res.ok) {
                        const blob = await res.blob();
                        const url = URL.createObjectURL(blob);
                        const a = document.createElement('a');
                        a.href = url;
                        a.download = `${(activeProfileData.name || 'profile').replace(/\s+/g, '_')}_profile.pdf`;
                        a.click();
                        URL.revokeObjectURL(url);
            ...
            async function exportBulkPDF() {
                if (!activeBulkData || !activeBulkData.length) { showSearchError('No bulk profiles loaded.'); return; }
                try {
                    const res = await fetch('/api/export-bulk-pdf', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ profiles: activeBulkData })
                    });
                    if (res.ok) {
                        const blob = await res.blob();
                        const url = URL.createObjectURL(blob);
